...
public class CommonNativeWrappers {
    ...
    public boolean launchApp(String platformName, String deviceName, String udid, String appPackage, String appActivity,
                             ...
                             String bundleId, String app, String mjpegServerPort, String wdaLocalPort, String webkitDebugProxyPort) {
        try {
...
//					return driver.findElement(AppiumBy.name(locValue));
    ...
    public void clickWebElementUsingCoOrdinates(WebElement ele) {
        String bounds = null;
        if (driver.getCapabilities().getPlatformName().toString().equalsIgnoreCase("Android")) {
            bounds = ele.getAttribute("bounds");
        } else if (driver.getCapabilities().getPlatformName().toString().equalsIgnoreCase("iOS")) {
        ...
        String[] eleStart = split[0].split(",");
        String[] eleEnd = split[1].split(",");
        int X = (int) ((Integer.parseInt(eleEnd[0]) - Integer.parseInt(eleStart[0])) * 0.5)
                + Integer.parseInt(eleStart[0]);
        int Y = (int) ((Integer.parseInt(eleEnd[1]) - Integer.parseInt(eleStart[1])) * 0.5)
                + Integer.parseInt(eleStart[1]);
        PointerInput finger = new PointerInput(Kind.TOUCH, "finger");
        Sequence sequence = new Sequence(finger, 1);
    ...
    public void clickWebElementUsingCoOrdinatesWithAdditionalValues(WebElement ele, int addX, int addY) {
        String bounds = null;
        if (driver.getCapabilities().getPlatformName().toString().equalsIgnoreCase("Android")) {
            bounds = ele.getAttribute("bounds");
        } else if (driver.getCapabilities().getPlatformName().toString().equalsIgnoreCase("iOS")) {
        ...
        PointerInput finger = new PointerInput(Kind.TOUCH, "finger");
        Sequence sequence = new Sequence(finger, 1);
        sequence.addAction(finger.createPointerMove(Duration.ZERO, Origin.viewport(), X + addX, Y + addY));
        sequence.addAction(finger.createPointerDown(MouseButton.LEFT.asArg()));
        sequence.addAction(new Pause(finger, Duration.ofMillis(200)));
        sequence.addAction(finger.createPointerUp(MouseButton.LEFT.asArg()));
        driver.perform(Arrays.asList(sequence));
